...
void VDBFusionMapper::mapIntegrateProcess(){
  while(ros::ok()){
    ...
    tsdf_volume.Integrate(points, color, origin, common::WeightFunction::constant_weight);
...
void VDBFusionMapper::filterptRange(const typename pcl::PointCloud<PCLPoint>& pointcloud_pcl, 
  ...
  int id = 0;
  for (auto item = pointcloud_pcl.begin(); item != pointcloud_pcl.end(); item++, id++) {
...
bool VDBFusionMapper::saveMap_callback(
  ...
  if(color_pointcloud && color_.size()!=0){
    ...
    for (size_t i = 0; i < vertices.size(); i++) {
      ...
      // std::cout << "r,g,b: " << color_[i][0] << " , " << color_[i][1] << " , " << color_[i][2] <<std::endl; // debug only
  ...
  open3d::geometry::TriangleMesh mesh_o3d = open3d::geometry::TriangleMesh(vertices, triangles);
  if (color_pointcloud && color_.size()!=0) {
    mesh_o3d.vertex_colors_.reserve(mesh_o3d.vertices_.size());
    for (size_t i = 0; i < mesh_o3d.vertices_.size(); i++) {
      mesh_o3d.vertex_colors_.emplace_back(color_[i][0] / 255.0,
                                            color_[i][1] / 255.0,
                                            color_[i][2] / 255.0);
    }
  }
  ...
  // if you don't want to use the open3d to save, use this one for simple but no RGB!  
